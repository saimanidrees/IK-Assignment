...
public class SCC_Drivetrain : MonoBehaviour {
    ...
    private void ApplySteering() {
        ...
        for (int i = 0; i < wheels.Length; i++) {
            ...
            {
                wheels[i].wheelCollider.WheelCollider.steerAngle =
                    (wheels[i].steeringAngle * InputProcessor.inputs.steerInput) *
                    Mathf.Lerp(1f, .25f, speed / highSpeedSteerAngle);
                if (!steeringWheel)
                    steeringWheel = transform.Find("Steering");
                steeringWheel.localRotation = Quaternion.Euler(24, 0, -wheels[0].wheelCollider.WheelCollider.steerAngle * 4f);
